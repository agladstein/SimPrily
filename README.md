...
pip install pip-tools
pip-sync
macss_env/bin/python simprily.py macsswig_examples/eg3/param_file_eg3.txt macsswig_examples/eg3/model_file_eg3.csv macsSwig 1 ill_650_test.bed 0 True output_dir
```
...
#### Output
Four subdirectories are created in either the current working directory or the directory specified in the `output_dir` argument. #change the name of the directories so they are not name AJ_M1
```text
